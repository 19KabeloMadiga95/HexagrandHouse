...
def read_cached_or_download():
    ...
    df = read_fixtures_url()

    if not df.empty:

        RAW_CACHE_FILE.parent.mkdir(
            parents=True,
            exist_ok=True
        )
        ...
        print(f"Fresh fixtures downloaded and cached")

        return df

    if RAW_CACHE_FILE.exists():

        print("Using cached fixtures file")
...
def enrich_league_fields(df):
    ...
    df["League"] = df["LeagueCode"].map(
        lambda x: LEAGUE_MAP.get(
            str(x),
            ("Unknown", "Unknown", "Unknown")
        )[0]
    )

    df["Country"] = df["LeagueCode"].map(
        lambda x: LEAGUE_MAP.get(
            str(x),
            ("Unknown", "Unknown", "Unknown")
        )[1]
...
def transform_fixtures(raw_df):
    ...
    df["FixtureDate"] = parse_fixture_date(
        df["FixtureDate"]
    )
    ...
    df["IngestedAt"] = datetime.now().strftime(
        "%Y-%m-%d %H:%M:%S"
    )
