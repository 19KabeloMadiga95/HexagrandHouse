...
FEATURES_FILE = BASE_DIR / "data" / "processed" / "features" / "uk49s_features.xlsx"

EXPORT_DIR = BASE_DIR / "data" / "exports" / "predictions"
OUTPUT_FILE = EXPORT_DIR / "uk49s_predictions.xlsx"
...
def load_uk49s_features(draw_type=None):
    ...
    if draw_type is not None:
        df = df[df["DrawType"] == draw_type].copy()
...
def generate_predictions_for_draw_type(
    draw_type,
    simulation_count=SIMULATION_COUNT,
    top_n=TOP_PREDICTIONS,
):
    df = load_uk49s_features(draw_type=draw_type)

    if df.empty:
        raise ValueError(f"No UK49s feature data found for draw type: {draw_type}")
    ...
    while len(candidates) < simulation_count and attempts < max_attempts:
        ...
        if regular_sum < 75 or regular_sum > 245:
            continue
    ...
    if not candidates:
        raise ValueError(f"No candidates generated for {draw_type}. Relax filters.")

    candidate_df = pd.DataFrame(candidates)

    candidate_df = candidate_df.drop_duplicates(
        subset=["DrawType", "N1", "N2", "N3", "N4", "N5", "N6", "Bonus"]
    )
    ...
    for rank, candidate in enumerate(selected_rows, start=1):
        ...
        final_rows.append({
            ...
            "ModelVersion": "UK49s_v1_statistical",
...
def generate_predictions(
    ...
    lunchtime = generate_predictions_for_draw_type(
        draw_type="Lunchtime",
        simulation_count=simulation_count,
        top_n=top_n,
    )

    teatime = generate_predictions_for_draw_type(
        draw_type="Teatime",
        simulation_count=simulation_count,
        top_n=top_n,
    )
...
def export_uk49s_predictions(
    ...
    with pd.ExcelWriter(
        ...
        predictions.to_excel(
            writer,
            sheet_name="UK49s_Predictions",
            index=False
        )

        predictions[predictions["DrawType"] == "Lunchtime"].to_excel(
            writer,
            sheet_name="Lunchtime",
            index=False
        )

        predictions[predictions["DrawType"] == "Teatime"].to_excel(
            writer,
            sheet_name="Teatime",
            index=False
        )

    print("\nUK49s predictions exported.")
