...
OUTPUT_FILE = EXPORT_DIR / "powerball_predictions.xlsx"
...
def load_powerball_features():
    ...
    number_cols = ["N1", "N2", "N3", "N4", "N5", "Bonus"]

    for col in number_cols:
        df[col] = pd.to_numeric(df[col], errors="coerce")

    df = df.dropna(subset=["DrawDate", "N1", "N2", "N3", "N4", "N5", "Bonus"])
...
def generate_predictions(
    ...
    attempts = 0
    max_attempts = simulation_count * 20
    ...
    for rank, candidate in enumerate(selected_rows, start=1):
        match = candidate_df[
            ...
            & (candidate_df["Bonus"] == candidate[5])
        ].iloc[0]
        ...
        final_rows.append({
            ...
            "ModelVersion": "PowerBall_v1_statistical",
...
def export_powerball_predictions(
    simulation_count=SIMULATION_COUNT,
    top_n=TOP_PREDICTIONS,
):
    EXPORT_DIR.mkdir(
    ...
    with pd.ExcelWriter(
        ...
        predictions.to_excel(
            writer,
            sheet_name="PowerBall_Predictions",
            index=False
        )

    print("\nPowerBall predictions exported.")
