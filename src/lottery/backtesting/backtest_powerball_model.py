...
OUTPUT_FILE = EXPORT_DIR / "powerball_backtest_results.xlsx"
...
def build_weighted_number_pool(train_df):
    ...
    for idx, row in train_df.iterrows():
        ...
        bonus_counts[int(row["Bonus"])] += recency_weight
...
def generate_model_predictions(train_df, prediction_count=PREDICTIONS_PER_DRAW):
    ...
    attempts = 0
    max_attempts = prediction_count * 100
    ...
    while len(predictions) < prediction_count and attempts < max_attempts:
        ...
        if regular_sum < 60 or regular_sum > 190:
            continue
    ...
    for test_idx in range(max_test_index):
        ...
        results.extend(
            score_predictions(
                model_predictions,
                actual_row,
                model_name="PowerBall_v1_weighted"
            )
        )
...
def export_powerball_backtest(
    ...
    results_df = run_powerball_backtest(
        game_name=game_name,
        test_draws=test_draws,
        predictions_per_draw=predictions_per_draw,
    )
