...
FEATURES_FILE = BASE_DIR / "data" / "processed" / "features" / "powerball_features.xlsx"

EXPORT_DIR = BASE_DIR / "data" / "exports" / "backtesting"
OUTPUT_FILE = EXPORT_DIR / "powerball_model_comparison_backtest.xlsx"
...
def load_powerball_features(game_name="PowerBall"):
    if not FEATURES_FILE.exists():
        raise FileNotFoundError(
            f"PowerBall features file not found:\n{FEATURES_FILE}\n\n"
            "Run this first:\n"
            "python -m src.lottery.features.powerball_features"
        )
    ...
    df = df[df["GameName"] == game_name].copy()
...
def apply_lookback(train_df, lookback_window):
    ...
    return train_df.head(lookback_window).copy()
...
def build_frequency_scores(train_df):
    ...
    reg_scores = pd.Series(
        {n: reg_counter[n] for n in REGULAR_RANGE}
    )
...
def build_overdue_scores(train_df):
    ...
    for n in REGULAR_RANGE:
        scores[n] = len(train_df) if last_seen[n] is None else last_seen[n]
...
def build_pair_scores(train_df):
    ...
    for _, row in train_df.iterrows():
        numbers = sorted(get_regular_list(row))
...
def build_number_weights(train_df, config):
    ...
    bonus_weights = (
        config["FrequencyWeight"] * freq_bonus
        + config["RecencyWeight"] * rec_bonus
    )

    reg_weights = reg_weights.clip(lower=0.001)
    bonus_weights = bonus_weights.clip(lower=0.001)

    return reg_weights.to_dict(), bonus_weights.to_dict()


def build_hot_numbers(train_df, top_n=10):
    counter = Counter()
...
def passes_basic_filters(numbers):
    ...
    if total < 60 or total > 190:
        return False

    if count_consecutive(numbers) > 2:
        return False
...
def generate_weighted_predictions(train_df, config, prediction_count):
    ...
    reg_weights, bonus_weights = build_number_weights(
        model_train_df,
        config
    )

    pair_counter = build_pair_scores(model_train_df)
    hot_numbers = build_hot_numbers(model_train_df, top_n=10)

    candidates = []
    seen = set()

    attempts = 0
    max_attempts = prediction_count * 500
    ...
    while len(candidates) < prediction_count * 50 and attempts < max_attempts:
        ...
        if config["Mode"] == "hybrid" and _rng.random() < hybrid_randomness:
            ...
            regulars = weighted_choice_no_replace(
                ...
                weights=[reg_weights[n] for n in REGULAR_RANGE],
                k=5
            )
        ...
        if _rng.random() < hybrid_randomness:
            ...
            bonus = weighted_choice_no_replace(
                ...
                weights=[bonus_weights[n] for n in BONUS_RANGE],
                k=1
            )[0]
        ...
        if not passes_basic_filters(regulars):
            continue

        seen.add(key)

        base_score = sum(reg_weights[n] for n in regulars) + bonus_weights[bonus]
        pair_score = score_pair_strength(regulars, pair_counter)
        pattern_score = score_pattern(regulars)
        penalty = hot_cluster_penalty(regulars, hot_numbers)
    ...
    for _, row in candidates_df.iterrows():
        ...
        for existing in selected:
            ...
            if len(candidate_set.intersection(existing_set)) > 3:
                ok = False
                break
...
def score_predictions(predictions_df, actual_row, model_name):
    ...
    for rank, pred in predictions_df.reset_index(drop=True).iterrows():
        ...
        regular_matches = len(
            predicted_regulars.intersection(actual_regulars)
        )

        bonus_match = 1 if int(pred["Bonus"]) == actual_bonus else 0

        total_score = regular_matches + bonus_match
...
def run_model_comparison_backtest(
    ...
    df = load_powerball_features(game_name=game_name)
    ...
    print("\n======================================")
    print("POWERBALL MODEL COMPARISON BACKTEST")
    print("======================================")
...
def build_model_summary(results_df):
    ...
    grouped = results_df.groupby("ModelName")
...
def build_rank_summary(results_df):
    return (
        results_df
        .groupby(["ModelName", "PredictionRank"])
        .agg(
...
def build_hit_distribution(results_df):
    return (
        results_df
        .groupby(["ModelName", "RegularMatches"])
        .agg(
...
def export_model_comparison_backtest(
    game_name="PowerBall",
    test_draws=TEST_DRAWS,
    predictions_per_draw=PREDICTIONS_PER_DRAW,
):
    ...
    results_df = run_model_comparison_backtest(
        game_name=game_name,
        test_draws=test_draws,
        predictions_per_draw=predictions_per_draw,
    )
    ...
    print("\nPowerBall model comparison backtest exported.")
