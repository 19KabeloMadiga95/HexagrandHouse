...
OUTPUT_FILE = (
    EXPORT_DIR
    / "uk49s_model_comparison_backtest.xlsx"
)
...
MODEL_CONFIGS = [
    ...
    {
        "ModelName": "AntiCrowding_Recent50",
        "Mode": "weighted",
        "LookbackWindow": 50,
        "FrequencyWeight": 1.5,
        "RecencyWeight": 2.0,
...
def build_frequency_scores(train_df):
    ...
    return (
        normalise_01(reg_scores),
        normalise_01(bonus_scores),
    )
...
def build_number_weights(
    ...
    freq_reg, freq_bonus = build_frequency_scores(
        train_df
    )

    rec_reg, rec_bonus = build_recency_scores(
        train_df
    )
    ...
    bonus_weights = bonus_weights.clip(
        lower=0.001
    )

    return (
        reg_weights.to_dict(),
        bonus_weights.to_dict(),
    )
    ...
    while (
        ...
        if _rng.random() < hybrid_randomness:
            ...
            bonus = weighted_choice_no_replace(
                pool=list(bonus_pool.keys()),
                weights=list(bonus_pool.values()),
                k=1
            )[0]
...
def run_uk49s_model_comparison_backtest(
    ...
    print("\n======================================")
    print("UK49S MODEL COMPARISON BACKTEST")
    print("======================================")
...
def export_uk49s_model_comparison_backtest(
    test_draws=TEST_DRAWS,
    predictions_per_draw=PREDICTIONS_PER_DRAW,
):
    EXPORT_DIR.mkdir(
    ...
    results_df = run_uk49s_model_comparison_backtest(
        test_draws=test_draws,
        predictions_per_draw=predictions_per_draw,
    )
    ...
    print("\nUK49s model comparison backtest exported.")
