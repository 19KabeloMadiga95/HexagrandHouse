...
OUTPUT_FILE = (
    EXPORT_DIR
    / "daily_lotto_backtest_results.xlsx"
)
...
def generate_model_predictions(
    ...
    regular_probs = build_weighted_number_pool(
        train_df
    )
    ...
    for test_idx in range(max_test_index):
        ...
        results.extend(
            score_predictions(
                ...
                model_name="DailyLotto_v1_weighted"
            )
        )
...
def export_daily_lotto_backtest(
    test_draws=TEST_DRAWS,
    predictions_per_draw=PREDICTIONS_PER_DRAW,
):
    EXPORT_DIR.mkdir(
    ...
    results_df = run_daily_lotto_backtest(
        test_draws=test_draws,
        predictions_per_draw=predictions_per_draw,
    )
